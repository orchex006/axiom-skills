#!/usr/bin/env python3
"""Claude Code `Stop` hook adapter for the Axiom graph workflow.
...
-----------------------------------
A `Stop` hook is not a user interrupt. An abort, a cancel, a host error or a hard kill is not
a stop event and is not claimed to invoke this hook reliably, so the adapter never treats an
interrupt as its own completion gate and never force-continues one. Only the documented
`Stop` event is handled.
...
* A bounded reconcile failure blocks only where the installed host actually supports a stop
  decision. When the reported capability is explicitly absent, the adapter reports `advisory`
  and allows the stop rather than claiming a gate it cannot enforce.
...
SUPPORTED_SCHEMA_PROFILES = ("claude-stop-v1",)
...
def canonical_decide(payload, *, cwd=None, path=None):
    ...
    if stop_reason in NEVER_FORCED_STOP_REASONS:
        return canonical_result("allow", f"stop reason {stop_reason} is never forced"), [
            f"stop reason {stop_reason!r} is never force-continued; a user interrupt is "
            "not claimed to invoke the Stop hook reliably"
        ]
    ...
    capability = supports_stop_decision(report)
    if capability is False:
        return (
            ...
            [
                ...
                "reconcile failure is advisory here and does not block"
            ],
        )
...
def to_host_output(canonical):
    """Map the canonical result to the documented Claude Code Stop hook output schema."""
